...
long long PrimeLoader::read_prime_from_file(const std::string &path) {
...
void PrimeLoader::load_from_files() {
    for (size_t i = 0; i < primes_.size(); i++) {
        ...
        if (fid >= 25 && fid <= 29) {
            ...
            if (p <= 0) {
                path = "25_29/" + std::to_string(fid) + "/"
                     + std::to_string(fid) + "_1.txt";
                p = read_prime_from_file(path);
            }
        }
        ...
        if (p > 0) {
            primes_[i].prime = p;
            snprintf(primes_[i].label, sizeof(primes_[i].label),
                     "%d (p=%lld)", fid, p);
...
void PrimeLoader::print_table() const {
    ...
    for (size_t i = 0; i < primes_.size(); i++) {
        if (primes_[i].prime > 0) {
            printf("  %2d -> %15lld  %s\n",
                   primes_[i].folder_id, primes_[i].prime, primes_[i].label);
        } else {
