...
// apm_brahma.cu
//
// APM Brahma — Single-file version with all improvements
//
...
static const int MIN_DEV = 5; // minimum deviation to test
static const int MAX_IDX_STATIC =
    50; // max indices in any IndexSet (supports up to 52×52)
static const int EARLY_STOP_HIT = 90; // early-stop threshold for matrices hit
...
static const struct {
  ...
  long long prime;
    ...
    {45, 35184372088777LL},
    {46, 70368744177643LL},
    {47, 140737488355213LL},
    {48, 281474976710597LL},
    {49, 562949953421231LL},
    {50, 1125899906842597LL},
};
static const int NUM_HARDCODED =
    sizeof(HARDCODED_PRIMES) / sizeof(HARDCODED_PRIMES[0]);
...
__device__ inline long long mod_mul(long long a, long long b, long long p,
  ...
  while (hi) {
    if (hi & 1)
      hi_mod = (hi_mod + hbase) % up;
    hbase =
        (hbase + hbase) % up; // addition-doubling: safe since hbase < p < 2^50
    hi >>= 1;
  }
  ...
  for (int col = 0; col < k; col++) {
    ...
    for (int row = col + 1; row < k; row++) {
      ...
      for (int j = col + 1; j < k; j++)
        a[row * k + j] =
            mod_sub(a[row * k + j], mod_mul(f, a[col * k + j], p, pow2_64), p);
      a[row * k + col] = 0LL;
    }
  }
  return det;
...
static void gen_combos(const vector<int> &pool, int r, const int *principal,
                       vector<IndexSet> &out) {
...
// =============================================================================
// GPU buffer manager — runtime VRAM detection via cudaMemGetInfo()
// =============================================================================
...
static void write_result_file(const string &out_dir, const MatrixData &md,
                              ...
                              const vector<ZeroMinor> &minors, double matrix_ms,
...
static void write_group_result(int group, int best_dev, int best_hits,
  ...
  fprintf(f, "============================================================\n");
  fprintf(f, "APM Early-Stop Result  (brahma)\n");
  fprintf(f, "============================================================\n");
...
static vector<ZeroMinor> search_matrix(GPUBufs &gpu, const long long *h_mat,
  ...
  unsigned long long pow2_64 = 1;
  {
    ...
    for (int i = 0; i < 64; i++)
      pow2_64 = (pow2_64 + pow2_64) % up; // addition-doubling, no overflow
  }
...
int main(int argc, char **argv) {
  ...
  printf("=================================================================\n");
  printf(" APM Brahma — Single-File First-Hit Search with Early Stop\n");
