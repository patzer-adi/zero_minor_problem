...
//     For EACH folder (antioffset, offset):
...
//   Results/
//     deviation_2/
//       offset/          ← <matrix_name>_result.txt for every matrix
//       antioffset/
//     deviation_3/
//       offset/
//       antioffset/
...
//        apm_research_3.cu -o apm3d              \
//        -Xlinker -lstdc++
//
// RUN (auto-detects both folders):
//   ./apm3d <prime>
//   ./apm3d <prime> <antioffset_dir> <offset_dir>
//   ./apm3d 1048573
//   ./apm3d 1048573 p_1048573_antioffset=0.2/ p_1048573_offset=1/
// =============================================================================
...
#define COL_CHUNK 512  // col-sets per GPU kernel launch (controls VRAM usage)
                       // 512 * C(58,5) * 12 bytes ≈ 27 GB → chunked safely
...
__device__ int det_mod(const int *mat, int k, int p) {
    ...
    for (int col = 0; col < k; col++) {
        ...
        if (piv != col) {
            for (int j = col; j < k; j++) {
                int tmp = a[col][j]; a[col][j] = a[piv][j]; a[piv][j] = tmp;
            }
...
//   (gridDim.z = 1, single matrix per launch)
// =============================================================================

__global__ void apm_kernel(
    const int       *d_matrix,       // n*n matrix (device)
    int              n,
...
static void mkdir_safe(const std::string &p) {
    mkdir(p.c_str(), 0755);
}
...
static MatrixData parse_matrix(const std::string &path) {
    ...
    if (rows.empty()) throw std::runtime_error("No matrix data in: " + path);
    md.n = (int)rows.size();
...
static void gen_combos(const std::vector<int> &pool, int r,
    ...
    while (true) {
        IndexSet is;
        is.k = PM_SIZE + r;
        for (int i = 0; i < PM_SIZE; i++) is.idx[i] = principal[i];
        for (int i = 0; i < r; i++)       is.idx[PM_SIZE + i] = pool[cidx[i]];
        ...
        if (carry < 0) break;
        cidx[carry]++;
        for (int i = carry + 1; i < r; i++) cidx[i] = cidx[i - 1] + 1;
    }
}
    ...
    double                        minors_tested)
{
    ...
    fprintf(f, "Full matrix (%dx%d) mod %d:\n", md.n, md.n, prime);
    // Column index header
    fprintf(f, "     ");
    ...
    if (minors.empty()) {
        ...
        for (int mi = 0; mi < (int)minors.size(); mi++) {
            ...
            fprintf(f, "  Principal block s: %d  (indices {%d, %d})\n",
                    zm.s, zm.s, zm.s + 1);
            // Write row indices — one space between numbers, no trailing space
            fprintf(f, "  Row indices [%d]  : ", zm.k);
            for (int j = 0; j < zm.k; j++)
                fprintf(f, "%d%s", zm.row_idx[j], j < zm.k - 1 ? " " : "");
            fprintf(f, "\n");
            // Write col indices — same format
            ...
            // ── Principal 2×2 block ───────────────────────────────────────────
            fprintf(f, "\n  Principal 2x2 block (s=%d, rows/cols {%d,%d}):\n",
                    zm.s, zm.s, zm.s + 1);
            fprintf(f, "    [ %*d  %*d ]\n",
            ...
            // ── Extracted k×k submatrix ───────────────────────────────────────
            // Find widest entry in just this submatrix for tighter formatting
            int sw = 1;
            ...
            // Column header: show actual matrix column indices
            fprintf(f, "       ");
            for (int c = 0; c < zm.k; c++) {
                // Mark principal block cols with '*'
            ...
            for (int r = 0; r < zm.k; r++) {
                // Mark principal block rows with '*'
                bool is_pm = (zm.row_idx[r] == zm.s || zm.row_idx[r] == zm.s + 1);
                fprintf(f, "  %3d%c |", zm.row_idx[r], is_pm ? '*' : ' ');
                for (int c = 0; c < zm.k; c++) {
                    int val = md.data[zm.row_idx[r] * md.n + zm.col_idx[c]];
                    // Mark the 2×2 principal block cell with brackets
...
struct GPUBufs {
    ...
    GPUBufs() : row_sets(NULL), col_sets(NULL), jobs(NULL),
                matrix(NULL), chunk_res(NULL), found_any(NULL),
                cap_rows(0), cap_jobs(0) {}
    ...
    void alloc(int n, int dev) {
        ...
        printf("    [GPU] Allocating: rows=%zu (%.1f MB)  jobs=%lld (%.1f MB)  "
               "matrix=%d×%d\n",
               cap_rows, cap_rows * sizeof(IndexSet) / 1e6,
               cap_jobs, cap_jobs * sizeof(MinorJob)  / 1e6,
               n, n);
        fflush(stdout);
    ...
    void free_all() {
        ...
        if (jobs)      { cudaFree(jobs);       jobs      = NULL; }
        if (matrix)    { cudaFree(matrix);     matrix    = NULL; }
        if (chunk_res) { cudaFree(chunk_res);  chunk_res = NULL; }
        if (found_any) { cudaFree(found_any);  found_any = NULL; }
    }
...
static std::vector<ZeroMinor> search_matrix(
    GPUBufs      &gpu,
    ...
    double        matrix_start_ms)
{
    ...
    // Upload matrix once
    CK(cudaMemcpy(gpu.matrix, h_mat,
                  (size_t)n * n * sizeof(int), cudaMemcpyHostToDevice));
    ...
    for (int s = 0; s <= n - PM_SIZE; s++) {
        ...
        if ((size_t)num_sets > gpu.cap_rows) {
            // Safety guard — shouldn't happen for dev≤5, n≤200
            fprintf(stderr,
        ...
        CK(cudaMemcpy(gpu.row_sets, sets.data(),
        ...
        for (int col_start = 0; col_start < num_sets; col_start += COL_CHUNK) {
            ...
            int         zero_fa = 0;
            CK(cudaMemcpy(gpu.chunk_res, &zero_cr, sizeof(ChunkResult), cudaMemcpyHostToDevice));
            CK(cudaMemcpy(gpu.found_any, &zero_fa, sizeof(int),         cudaMemcpyHostToDevice));

            // Upload col chunk and jobs
            CK(cudaMemcpy(gpu.col_sets,
                          sets.data() + col_start,
                          num_cols * sizeof(IndexSet),
                          cudaMemcpyHostToDevice));
            CK(cudaMemcpy(gpu.jobs, h_jobs.data(),
                          num_jobs * sizeof(MinorJob),
                          cudaMemcpyHostToDevice));
            ...
            ChunkResult h_cr;
            int         h_fa;
            CK(cudaMemcpy(&h_cr, gpu.chunk_res, sizeof(ChunkResult), cudaMemcpyDeviceToHost));
...
static void run_folder(
    ...
    const std::string &folder_tag,   // "offset" or "antioffset"
    int                prime,
    int                dev)
{
    ...
    printf("  ║  Deviation %d  |  %-12s  |  %zu matrices             \n",
           dev, folder_tag.c_str(), files.size());
    printf("  ║  Input : %s\n", folder_path.c_str());
    printf("  ║  Output: %s\n", out_dir.c_str());
    printf("  ╚══════════════════════════════════════════════════════════╝\n");
    fflush(stdout);
    ...
    // NOTE: opened AFTER all early returns so we never leak the handle
    ...
    for (int fi = 0; fi < (int)files.size(); fi++) {
        ...
        if (found.empty()) {
            ...
            for (int mi = 0; mi < (int)found.size(); mi++) {
                ...
                printf("    [Zero #%d] size=%d  dev=%d  s=%d  t=%.2f ms\n",
                       mi + 1, zm.k, zm.dev, zm.s, zm.time_ms);
                printf("      Rows [%d]: ", zm.k);
                for (int j = 0; j < zm.k; j++)
        ...
        write_result_file(out_dir, md, prime, dev,
                          found, matrix_ms, minors_tested);
    ...
    if (sum_f) {
        ...
        // BUG FIX: fclose is now unconditional — always flush & close
        fclose(sum_f);
        sum_f = NULL;
        printf("  Summary -> %s\n", sum_path.c_str());
    }
...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr,
            // BUG FIX: usage string now matches the actual argv order
            // (argv[2]=antioffset, argv[3]=offset) to match Phase 1 / Phase 2 order
            "Usage: %s <prime> [antioffset_dir] [offset_dir]\n"
            "  %s 1048573\n"
            "  %s 1048573 p_1048573_antioffset=0.2/ p_1048573_offset=1/\n",
    ...
    // argv[2] = antioffset dir (Phase 1), argv[3] = offset dir (Phase 2)
    std::string dir_antioffset = (argc >= 3) ? argv[2] : "p_1048573_antioffset=0.2";
    std::string dir_offset     = (argc >= 4) ? argv[3] : "p_1048573_offset=1";

    // Auto-detect missing folders — scan current directory
    // BUG FIX: check each dir independently so a valid dir_antioffset is never
    // overwritten by the scan just because dir_offset happens to be missing.
    {
        struct stat st;
        ...
        if (!off_ok || !anti_ok) {
            ...
            if (d) {
                ...
                while ((e = readdir(d)) != NULL) {
                    std::string nm = e->d_name;
                    // Only update the dir that is actually missing
                    if (!off_ok &&
                        nm.find("antioffset") == std::string::npos &&
    ...
    // ── Program header ────────────────────────────────────────────────────────
    printf("=================================================================\n");
    printf(" APM Exhaustive Search — All Deviations, All Matrices, All Minors\n");
    printf(" TWO-PHASE: ALL antioffset (dev %d->%d) THEN ALL offset (dev %d->%d)\n",
           MIN_DEV, MAX_DEV, MIN_DEV, MAX_DEV);
    printf("=================================================================\n");
    printf(" Prime             : %d\n",   prime);
    ...
    printf("\n Phase 1 - antioffset: Results/deviation_{%d..%d}/antioffset/\n",
           MIN_DEV, MAX_DEV);
    printf(" Phase 2 - offset    : Results/deviation_{%d..%d}/offset/\n",
           MIN_DEV, MAX_DEV);
    printf("=================================================================\n\n");
    fflush(stdout);
    ...
    // =========================================================================
    // PHASE 1 — ANTIOFFSET: all deviations 2 -> 5, every matrix, every minor
    //   Antioffset matrices are small (12x12) so this phase finishes quickly.
    ...
    // =========================================================================
    {
        ...
        printf("\n");
        printf("================================================================\n");
        printf("  PHASE 1 - ANTIOFFSET  (deviations %d -> %d, all matrices)\n",
               MIN_DEV, MAX_DEV);
        printf("  Folder : %s\n", dir_antioffset.c_str());
        printf("================================================================\n");
        fflush(stdout);
        ...
        if (!anti_ok) {
            ...
            for (int dev = MIN_DEV; dev <= MAX_DEV; dev++) {
                ...
                fflush(stdout);
                run_folder(dir_antioffset, "antioffset", prime, dev);
                printf("\n  [antioffset deviation_%d done]\n", dev);
                fflush(stdout);
            }
            double phase1_ms = now_ms() - phase1_start;
            printf("\n================================================================\n");
            printf("  PHASE 1 COMPLETE - ANTIOFFSET  (%.2f min)\n",
                   phase1_ms / 60000.0);
            printf("================================================================\n");
            fflush(stdout);
        }
    ...
    // =========================================================================
    // PHASE 2 — OFFSET: all deviations 2 -> 5, every matrix, every minor
    //   Offset matrices are larger so this phase takes longer.
    ...
    // =========================================================================
    {
        ...
        printf("\n");
        printf("================================================================\n");
        printf("  PHASE 2 - OFFSET  (deviations %d -> %d, all matrices)\n",
               MIN_DEV, MAX_DEV);
        printf("  Folder : %s\n", dir_offset.c_str());
        printf("================================================================\n");
        fflush(stdout);
        ...
        if (!off_ok) {
            ...
            for (int dev = MIN_DEV; dev <= MAX_DEV; dev++) {
                printf("\n  -- OFFSET  deviation=%d  minor size=%dx%d --\n",
                       dev, PM_SIZE + dev, PM_SIZE + dev);
                fflush(stdout);
                run_folder(dir_offset, "offset", prime, dev);
                printf("\n  [offset deviation_%d done]\n", dev);
                fflush(stdout);
            }
            double phase2_ms = now_ms() - phase2_start;
            printf("\n================================================================\n");
            printf("  PHASE 2 COMPLETE - OFFSET  (%.2f min)\n",
                   phase2_ms / 60000.0);
            printf("================================================================\n");
            fflush(stdout);
        }
    ...
    for (int d = MIN_DEV; d <= MAX_DEV; d++)
        printf("   Results/deviation_%d/antioffset/   Results/deviation_%d/offset/\n", d, d);
    printf("=================================================================\n");
