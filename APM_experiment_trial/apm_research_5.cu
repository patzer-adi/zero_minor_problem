...
//   Within each deviation, folders are processed in this fixed order:
//
//     deviation 2:
//       1. p_1048573 antioffset   (12x12 matrices, fast)
//       2. 25_29/25               (prime=33554393)
//       3. 25_29/26               (prime=44923183)
//       4. 25_29/27               (prime=134217689)
//       5. 25_29/28               (prime=268435399)
//       6. 25_29/29               (prime=536870909)
//       7. p_1048573 offset       (larger matrices, slow)
//
//     deviation 3:  same order — antioffset, 25, 26, 27, 28, 29, offset
//     deviation 4:  same order
//     deviation 5:  same order
//
//   For each folder at each deviation:
//     For EACH matrix file in the folder:
...
//     Write one SUMMARY.txt for the folder
//
//   Files ending in _RN.txt are IGNORED in all folders.
//
...
//     deviation_3/ deviation_4/ deviation_5/  (same sub-structure)
//
//     25/                    <- ALL deviations for prime=33554393
//       deviation_2/
//       deviation_3/
//       deviation_4/
//       deviation_5/
//     26/ 27/ 28/ 29/        <- same pattern for remaining primes
//
...
// Create Results/deviation_<d>/<tag>/ — used for antioffset and offset.
...
// Create Results/<folder_id>/deviation_<d>/ — used for 25_29 subfolders.
// Groups ALL deviation results for one prime under a single named folder,
// so e.g. Results/25/deviation_2/, Results/25/deviation_3/, ... are siblings.
static std::string make_out_dir_25(const std::string &folder_id, int dev) {
  mkdir_safe("Results");
  std::string d1 = "Results/" + folder_id;
  mkdir_safe(d1);
...
search_matrix(GPUBufs &gpu,
              ...
              int n,
              int prime, // cast to int for kernel (safe: values < 2^30)
...
static void run_folder(
    ...
    const std::string &folder_tag, // "offset", "antioffset", "25", "26", etc.
    long long prime, int dev, const std::string &out_dir_override = "") {
  // Use override path when provided (25_29 subfolders), else default layout.
  std::string out_dir = out_dir_override.empty() ? make_out_dir(dev, folder_tag)
                                                 : out_dir_override;
...
int main(int argc, char **argv) {
  ...
  struct stat st;
  bool anti_exists = (stat(dir_antioffset.c_str(), &st) == 0);
  bool offset_exists = (stat(dir_offset.c_str(), &st) == 0);
  bool base25_exists = (stat(dir_25_29.c_str(), &st) == 0);

  // ── Program header ────────────────────────────────────────────────────────
  printf("=================================================================\n");
  printf(" APM Exhaustive Search — apm_research_5\n");
  printf(" UNIFIED DEVIATION-OUTER LOOP\n");
  printf("\n");
  printf(" Execution order per deviation (2 -> 3 -> 4 -> 5):\n");
  printf("   1. p_1048573 antioffset   (%s)\n", dir_antioffset.c_str());
  printf("   2. 25_29/25              (prime=33554393)\n");
  printf("   3. 25_29/26              (prime=44923183)\n");
  printf("   4. 25_29/27              (prime=134217689)\n");
  printf("   5. 25_29/28              (prime=268435399)\n");
  printf("   6. 25_29/29              (prime=536870909)\n");
  printf("   7. p_1048573 offset       (%s)\n", dir_offset.c_str());
  printf("\n");
  printf(" PM block size    : %d (fixed)\n", PM_SIZE);
  printf(" Deviation levels : %d -> %d\n", MIN_DEV, MAX_DEV);
  printf(" COL_CHUNK        : %d\n", COL_CHUNK);
  printf(" p_1048573 prime  : %lld\n", P_1048573);
  printf(" antioffset found : %s\n", anti_exists ? "YES" : "NO (will skip)");
  printf(" offset found     : %s\n", offset_exists ? "YES" : "NO (will skip)");
  printf(" 25_29 found      : %s\n", base25_exists ? "YES" : "NO (will skip)");
  printf(
      "=================================================================\n\n");
  fflush(stdout);
  ...
  //   Step 1: p_1048573 antioffset  -> Results/deviation_<d>/antioffset/
  //   Step 2: 25_29/25             -> Results/25/deviation_<d>/
  //   Step 3: 25_29/26             -> Results/26/deviation_<d>/
  //   Step 4: 25_29/27             -> Results/27/deviation_<d>/
  //   Step 5: 25_29/28             -> Results/28/deviation_<d>/
  //   Step 6: 25_29/29             -> Results/29/deviation_<d>/
  //   Step 7: p_1048573 offset     -> Results/deviation_<d>/offset/
  // =========================================================================
  ...
  for (int dev = MIN_DEV; dev <= MAX_DEV; dev++) {
    ...
    printf("# DEVIATION %d   minor size = %dx%d\n", dev, PM_SIZE + dev,
           PM_SIZE + dev);
    printf("# Order: antioffset -> 25 -> 26 -> 27 -> 28 -> 29 -> offset\n");
    printf(
        "#################################################################\n");
    fflush(stdout);
    ...
    // ── Step 1: p_1048573 antioffset ─────────────────────────────────────
    printf("\n");
    printf(
        "  ================================================================\n");
    printf("  [dev=%d | 1/7] p_1048573 ANTIOFFSET\n", dev);
    printf("  Folder : %s\n", dir_antioffset.c_str());
    printf("  Prime  : %lld\n", P_1048573);
    printf(
        "  ================================================================\n");
    ...
    if (!anti_exists) {
      printf("  [SKIP] folder not found: %s\n", dir_antioffset.c_str());
    } else {
      run_folder(dir_antioffset, "antioffset", P_1048573, dev);
      printf("  [dev=%d antioffset done]\n", dev);
    }
    fflush(stdout);

    // ── Steps 2-6: 25_29 subfolders ──────────────────────────────────────
    if (!base25_exists) {
      printf("\n  [SKIP] 25_29/ directory not found: %s\n", dir_25_29.c_str());
    } else {
      for (int fi = 0; fi < NUM_FOLDER_PRIMES; fi++) {
        ...
        printf("  "
               "==============================================================="
               "=\n");
        printf("  [dev=%d | %d/7] 25_29/%d   prime=%lld\n", dev, fi + 2, fid,
               fprime);
        printf("  Folder : %s\n", sub_dir.c_str());
        printf("  "
               "==============================================================="
        ...
        std::string out_dir = make_out_dir_25(tag, dev);
        run_folder(sub_dir, tag, fprime, dev, out_dir);
        printf("  [dev=%d  25_29/%d done]\n", dev, fid);
        fflush(stdout);
      }
    }

    // ── Step 7: p_1048573 offset ──────────────────────────────────────────
    printf("\n");
    printf(
        "  ================================================================\n");
    printf("  [dev=%d | 7/7] p_1048573 OFFSET\n", dev);
    printf("  Folder : %s\n", dir_offset.c_str());
    printf("  Prime  : %lld\n", P_1048573);
    printf(
        "  ================================================================\n");
  ...
  for (int d = MIN_DEV; d <= MAX_DEV; d++) {
    ...
    for (int i = 0; i < NUM_FOLDER_PRIMES; i++)
      printf("   Results/%d/deviation_%d/\n", FOLDER_PRIMES[i].folder_id, d);
