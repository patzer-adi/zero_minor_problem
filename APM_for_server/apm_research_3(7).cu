...
//
// Execution order (two-phase):
//
//   PHASE 1 — ANTIOFFSET  (all deviations 2→5, completes entirely first)
//     For EACH deviation D in {2, 3, 4, 5}:
//       For EACH matrix in antioffset folder:
//         For EACH principal block position s (0 to n-2):
//           For EVERY (row_set × col_set) pair at deviation D:
//             Compute det(submatrix) mod prime
//             If det == 0  →  record it  (NEVER stop early, find ALL)
//         Write one result file per matrix
//       Write SUMMARY.txt for this deviation
//
//   PHASE 2 — OFFSET  (all deviations 2→5, starts only after Phase 1 done)
//     Same procedure for offset folder (larger matrices, takes longer)
//
// Rationale: antioffset matrices are 12×12 (fast); offset matrices are
//   much larger. Running all antioffset first means quick, complete results
//   are on disk before the long offset computation begins.
//
// Output folder structure created automatically:
//   Results/
//     deviation_2/
//       antioffset/      ← <matrix_name>_result.txt + SUMMARY.txt
//       offset/
//     deviation_3/
//       antioffset/
//       offset/
...
//        -Xlinker -lstdc++
//
// RUN (auto-detects both folders, antioffset processed first):
//   ./apm3d <prime>
...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr,
            "Usage: %s <prime> [offset_dir] [antioffset_dir]\n"
            "  %s 1048573\n"
            "  %s 1048573 p_1048573_offset=1/ p_1048573_antioffset=0.2/\n",
    ...
    // Auto-detect if defaults don't exist — scan current directory
    {
        struct stat st;
    ...
    printf(" TWO-PHASE: ALL antioffset (dev 2->5) THEN ALL offset (dev 2->5)\n");
    printf("=================================================================\n");
    printf(" Prime             : %d\n",   prime);
    ...
    printf("\n Phase 1 - antioffset results: Results/deviation_{%d..%d}/antioffset/\n",
           MIN_DEV, MAX_DEV);
    printf(" Phase 2 - offset results    : Results/deviation_{%d..%d}/offset/\n",
           MIN_DEV, MAX_DEV);
    printf("=================================================================\n\n");
    fflush(stdout);
    ...
    //   Antioffset matrices are small (12×12) so this phase finishes quickly.
    ...
    // =========================================================================
    {
        ...
        if (!anti_ok) {
            ...
            for (int dev = MIN_DEV; dev <= MAX_DEV; dev++) {
                printf("\n");
                printf("  ┌─────────────────────────────────────────────────────┐\n");
                printf("  │  ANTIOFFSET  deviation=%d  minor size=%dx%d          │\n",
    ...
    // =========================================================================
    {
        ...
        if (!off_ok) {
            ...
            for (int dev = MIN_DEV; dev <= MAX_DEV; dev++) {
                printf("\n");
                printf("  ┌─────────────────────────────────────────────────────┐\n");
                printf("  │  OFFSET  deviation=%d  minor size=%dx%d              │\n",
                       dev, PM_SIZE + dev, PM_SIZE + dev);
                printf("  └─────────────────────────────────────────────────────┘\n");
                fflush(stdout);
