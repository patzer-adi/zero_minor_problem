...
// apm_research_6.cu
//
// APM — Almost Principal Minors, Exhaustive Search (V6 — Production Rewrite)
//
...
//
// Key improvements over V5 / V5_new:
//   - NO MinorJob struct/array — threads derive (r,c) from tid
//   - NO ChunkResult / atomicCAS — uses d_zero_flags[] to capture ALL zeros
//   - NO d_found_any — flags array encodes everything
...
//        apm_research_6.cu -o apm6 -Xlinker -lstdc++
//
// =============================================================================
...
// ALL zeros are captured — no early stopping.
// =============================================================================
...
static std::string make_out_dir(int group, int dev) {
  mkdir_safe("Results_6");
  std::string d1 = "Results_6/" + std::to_string(group);
  mkdir_safe(d1);
...
static void write_zero_minor_detail(FILE *f, const ZeroMinor &zm,
  ...
  for (int r = 0; r < zm.k; r++)
    for (int c = 0; c < zm.k; c++) {
      long long v = md.data[zm.row_idx[r] * md.n + zm.col_idx[c]];
      int w = 1;
      if (v < 0) {
        v = -v;
        w = 2;
...
search_matrix(GPUBufs &gpu,
  ...
  for (int s = 0; s <= n - PM_SIZE; s++) {
    ...
    for (int col_start = 0; col_start < num_sets; col_start += chunk_sz) {
      ...
      // Copy flags back and scan ALL of them
...
int main(int argc, char **argv) {
  ...
  // ── Program header ──────────────────────────────────────────────────────
  printf("=================================================================\n");
  printf(" APM Exhaustive Search — apm_research_6\n");
  ...
  for (int gi = 0; gi < NUM_FOLDER_PRIMES; gi++) {
    ...
    for (int d = MIN_DEV; d <= md; d++)
      printf("   Results_6/%d/deviation_%d/\n", g, d);
  }
  printf("=================================================================\n");
