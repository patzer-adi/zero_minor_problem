// =============================================================================
// apm_research_hits_one_check_till_100.cu
//
// APM — Almost Principal Minors, First-Hit Search — EARLY STOP AT 100 HITS
//
// Based on apm_research_hits_one.cu.  Same first-hit-per-matrix logic, but
// adds an EARLY-STOP rule across deviations within each prime group:
//
//   For each prime group, iterate deviations 2, 3, ..., n-3 as usual.
//   At each deviation, count how many of the 100 matrices got a hit
//   ("matrices_hit").  As soon as matrices_hit == 100 (ALL matrices hit),
//   STOP — do NOT check any further deviations for this group.
//   Write  Results_hits_one_check_till_100/<group>/result.txt  recording
//   which deviation achieved 100 hits, then move on to the next group.
//
//   If no deviation reaches 100, the best deviation (highest hit count)
//   is recorded in result.txt instead.
//
// Key properties (inherited from apm_research_hits_one / V6):
//   - EARLY EXIT per matrix: stops after FIRST zero minor per matrix
...
//        apm_research_hits_one_check_till_100.cu \
//        -o apm_hits_one_check_till_100 -Xlinker -lstdc++
//
// =============================================================================
...
// Create Results_hits_one_check_till_100/<group>/deviation_<d>/
static std::string make_out_dir(int group, int dev) {
  mkdir_safe("Results_hits_one_check_till_100");
  std::string d1 = "Results_hits_one_check_till_100/" + std::to_string(group);
  mkdir_safe(d1);
  ...
  if (minors.empty()) {
    ...
    for (int mi = 0; mi < (int)minors.size(); mi++) {
      ...
      fprintf(f, "  Time found       : %.4f ms  (since matrix start)\n",
              zm.time_ms);
      fprintf(f, "  Minors tested    : %.0f  (tested before finding this zero)\n",
              minors_tested);
      fprintf(f, "\n");
...
// Returns: matrices_hit (number of matrices that had at least one zero minor)
// =============================================================================
...
// =============================================================================
// Helper: write result.txt for a prime group
// =============================================================================

static void write_group_result(int group, int best_dev, int best_hits,
                               bool reached_100) {
  mkdir_safe("Results_hits_one_check_till_100");
  std::string dir = "Results_hits_one_check_till_100/" + std::to_string(group);
  mkdir_safe(dir);
  ...
  if (reached_100) {
    ...
    fprintf(f, "Matrices hit     : %d  (reached 100 — EARLY STOP)\n",
            best_hits);
    ...
    fprintf(f, "Best deviation   : %d\n", best_dev);
    fprintf(f, "Matrices hit     : %d  (did NOT reach 100)\n", best_hits);
...
int main(int argc, char **argv) {
  ...
  // ── Program header ──────────────────────────────────────────────────────
  printf("=================================================================\n");
  printf(" APM First-Hit Search — check_till_100\n");
  printf(" EARLY STOP: skips remaining deviations once 100 hits reached\n");
  ...
  //     If matrices_hit >= 100 -> write result.txt, STOP, next group
  //   If loop finishes without hitting 100 -> write result.txt with best dev
  // =========================================================================
  ...
  for (int gi = 0; gi < NUM_FOLDER_PRIMES; gi++) {
    ...
    if (files.empty()) {
      ...
      if (!files.empty()) {
        printf("  [INFO] No files matching prefix '%s'. Using prefix '%s' "
               "instead.\n",
    ...
    for (int dev = MIN_DEV; dev <= max_dev; dev++) {
      ...
      if (hits >= 100) {
        printf("\n  *** 100 HITS REACHED at deviation %d! ***\n", dev);
    ...
    if (best_dev >= 0) {
      write_group_result(group, best_dev, best_hits, reached_100);
    }
    ...
    if (reached_100) {
      printf("# EARLY STOP at deviation %d (100 hits)\n", best_dev);
    } else {
  ...
  printf(" Total wall time : %.3f s  (%.2f min)\n", total_ms / 1000.0,
         total_ms / 60000.0);
  printf(
      "\n Results in: Results_hits_one_check_till_100/<group>/\n");
  printf(
      " Each group has a result.txt with the best deviation.\n");
  printf("=================================================================\n");
