...
// This is cheaper than determinant check and used as a filter.
    ...
    for (size_t i = 0; i < n; ++i)
        ...
        for (size_t col = 0; col < n; ++col) {
            ...
            for (size_t row = col + 1; row < n; ++row) {
                ...
                long factor = ( (long)mat[row][col] * inv_pivot ) % p;
                for (size_t j = col; j < n; ++j) {
                    long t = (mat[row][j] - (factor * mat[col][j]) % p) % p;
                    if (t < 0) t += p;
                    mat[row][j] = t;
                }
            }
    ...
    for (long n = upper_bound; n >= 2; --n) {
        ...
        for (int start = rank; start < R && !global_found; start += size) {
            ...
            if (search_non_singular_dfs((int)n, p, rows, masks, chosen, used_rows, used_values, &global_found)) {
                local_found = 1;
                best_indices = chosen; // chosen contains the found full path because recursion returns true without backtracking
                break;
            }
        }
