...
private:
    ...
    std::set<long> used_elements;   // FIXED
    mt19937& gen;
...
public:
    ...
    bool canCompleteRow(int row, int col, long remaining_sum, const std::set<long>& temp_used) {
        if (col == n) {
    ...
    bool generateMatrix(int row, int col, long current_row_sum, std::set<vector<long>>& used_rows) {
        if (row == n) {
        ...
        std::set<long> temp_row_used;  // FIXED
        for (int j = 0; j < col; j++) {
    ...
    bool generate(Mat<ZZ>& result) {
        ...
        std::set<vector<long>> used_rows;  // FIXED
...
bool verifyMatrix(const Mat<ZZ>& matrix, int n, long P, long target_sum) {
    std::set<long> all_elements;   // FIXED
    ...
    std::set<vector<long>> row_set;   // FIXED
    for (int i = 0; i < n; i++) {
        vector<long> row;
        for (int j = 0; j < n; j++) {
...
int main() {
    ...
    cout << "Analysis: For 16 unique elements with each row sum = (P-1):" << endl;
