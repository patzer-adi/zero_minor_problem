...
// --- Generate all permutations of a combination (helper) ---
void generate_permutations(vector<long>& combination, long l, long r,
                           vector<Vec<ZZ_p>>& rows) {
    if (l == r) {
        Vec<ZZ_p> row;
        row.SetLength(combination.size());
        for (long i = 0; i < combination.size(); i++)
            row[i] = conv<ZZ_p>(combination[i]);
        rows.push_back(row);
        return;
    }

    for (long i = l; i <= r; i++) {
        swap(combination[l], combination[i]);
        generate_permutations(combination, l + 1, r, rows);
        swap(combination[l], combination[i]); // backtrack
    }
}

// --- Recursive function to generate unique combinations ---
void generate_combinations(long n, long p, long target_sum, long start,
    ...
    if (current.size() == n) {
        ...
        if (sum == target_sum) {
            ...
            generate_permutations(current, 0, n - 1, rows);
        }
        return;
    }
    ...
    for (long i = start; i <= p - 1; i++) {
        ...
        current.push_back(i);
        generate_combinations(n, p, target_sum, i + 1, current, rows);
        current.pop_back();
    }
}
...
int main() {
    ...
    cout << "Generating rows (with permutations) with sum=" << target_sum << "...\n";
    vector<Vec<ZZ_p>> rows;
    vector<long> current;
    generate_combinations(n, p, target_sum, 1, current, rows);
    generated_rows = rows;
    cout << "Total rows generated (including permutations): " << rows.size() << "\n";
