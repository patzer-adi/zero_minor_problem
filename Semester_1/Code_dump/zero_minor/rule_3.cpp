...
// Generate all combinations of `n` numbers in [1, p-1] whose sum is exactly `target_sum`
void generate_rows_recursive(long n, long p, long target_sum, vector<long>& current, vector<Vec<ZZ_p>>& rows, long start=1) {
    if (current.size() == n) {
        long sum = 0;
        ...
        if (sum == target_sum) {
            Vec<ZZ_p> row;
            row.SetLength(n);
            for (long i = 0; i < n; i++) row[i] = ZZ_p(current[i]);
            rows.push_back(row);
        }
        return;
    }
    ...
    for (long i = start; i <= p - 1; i++) {
        // Avoid duplicates in row
        bool used = false;
        for (auto x : current) if (x == i) { used = true; break; }
        if (used) continue;

        current.push_back(i);
        generate_rows_recursive(n, p, target_sum, current, rows, 1); // start=1 because order does not matter
        current.pop_back();
    }
}
...
// Check if new row conflicts with current matrix elements
...
int main() {
    ...
    vector<long> current;
    generate_rows_recursive(n, p, conv<long>(target_sum), current, rows);
    generated_rows = rows;
