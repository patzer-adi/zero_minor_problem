...
// Function to generate all possible rows with sum = p-1 and unique elements (1 to p-1)
vector<Vec<ZZ_p>> generate_rows(long n, ZZ_p p_minus_one) {
    ...
    while (true) {
        ...
        if (sum == p_minus_one) rows.push_back(row);

        long i = n - 1;
        while (i >= 0 && indices[i] == p - n + i + 1) i--;
        if (i < 0) break;
        indices[i]++;
...
void generate_matrices(long n, const vector<Vec<ZZ_p>>& rows, 
    ...
    if (current_matrix.size() == n) {
        ...
        if (all_elements_unique(mat)) {
            ...
            matrix_file << "Matrix #" << matrices_generated 
    ...
    for (long i = 0; i < rows.size() && !stop_generation; i++) {
        if (!used_row_indices[i] && !has_conflict(current_matrix, rows[i])) {
            ...
            generate_matrices(n, rows, current_matrix, used_row_indices, 
                              max_matrices, matrices_generated, stop_generation, matrix_file);
...
int main() {
    ...
    if (n > p - 1) {  // Because elements are 1..p-1
        cout << "Error: Matrix size n cannot be larger than p-1." << endl;
        return 1;
    }
    ...
    for (long i = 0; i < generated_rows.size(); i++) {
        row_file << "Row " << i+1 << ": ";
        ZZ_p sum = conv<ZZ_p>(0);
        for (long j = 0; j < n; j++) {
            row_file << generated_rows[i][j] << " ";
            sum += generated_rows[i][j];
        }
        row_file << "Sum=" << sum << "\n";
    }
    row_file.close();
    ...
    cout << "\nTotal matrices generated: " << matrices_generated << endl;
