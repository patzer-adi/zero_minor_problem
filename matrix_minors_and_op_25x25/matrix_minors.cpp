...
void generate_combinations(long n, long k, vector<vector<long>> &combs, vector<long> current = {}, long start = 0) {
    ...
    for (long i = start; i < n; i++) {
        current.push_back(i);
        generate_combinations(n, k, combs, current, i + 1); //recursively add with index
        current.pop_back();
    }
}
...
int main() {
    long n = 25;  // Size of the matrix (adjust as needed)
    Mat<ZZ> A;
    A.SetDims(n, n);
    ...
    for (long k = 2; k <= 9; k++) {  // Be cautious with large k!
