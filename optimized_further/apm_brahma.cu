// =============================================================================
// apm_brahma.cu  — OPTIMIZED FURTHER
//
// APM Brahma — V100-OPTIMIZED, TEMPLATED, FLAT-ARRAY, KERNEL-RANGE VERSION
//
// Enhancements over optimized_flatarray:
//   1. KERNEL RANGE ARGS: kmin/kmax to select which kernel files to process.
//      Enables multi-GPU parallelism: split 100 kernels across GPUs.
//   2. NUMERICAL FILE SORTING: collect_files sorts by number (1,2,...100)
//      not lexicographically (1,10,100,11...). Works with both padded
//      (kernel_38_0001.txt) and unpadded (kernel_38_1.txt) filenames.
//   3. __int128 MOD_MUL: uses unsigned __int128 for single-instruction
//      modular multiply on SM>=70. Falls back to __umul64hi loop otherwise.
//
// Inherited V100 optimizations:
//   - Templated kernel apm_kernel_k<K>: sub[K*K] exact stack frame
//   - __constant__ pow2_64 (L1-cached, zero param overhead)
//   - Flat int[] index arrays (5-8x VRAM savings vs IndexSet)
//   - reserve() host vectors, graceful OOM skip
//
// Usage:
//   ./apm_brahma [gmin gmax]             (all kernels)
//   ./apm_brahma [gmin gmax kmin kmax]   (kernel file range)
//
// Examples:
//   ./apm_brahma 38 38                   # group 38, ALL kernels
//   ./apm_brahma 38 38 1 35              # group 38, kernels 1-35
//   ./apm_brahma 38 38 36 70             # group 38, kernels 36-70
//   ./apm_brahma 38 38 71 100            # group 38, kernels 71-100
//
// Multi-GPU:
//   CUDA_VISIBLE_DEVICES=0 ./apm_brahma 38 38  1 50 &
//   CUDA_VISIBLE_DEVICES=1 ./apm_brahma 38 38 51 100 &
//
// =============================================================================
...
static const char *RESULT_BASE_DIR = "Results_further";
...
//
// SM >= 700 (V100+): uses unsigned __int128 — single multiply, no loop.
// SM <  700 (fallback): uses __umul64hi intrinsic + binary loop.
//
// NOTE: __int128 in CUDA device code is compiler-dependent.
//   Tested on: nvcc 11.x+ with V100 (SM 70), A100 (SM 80).
//   If __int128 fails on your GPU (e.g. RTX 2080, SM 75), change the
//   threshold below to __CUDA_ARCH__ >= 800.
//   Run test_int128.cu to verify before deploying.
//
// __CUDA_ARCH__ is set by nvcc based on -gencode — automatic dispatch.
__device__ inline long long mod_mul(long long a, long long b, long long p) {
  ...
  // V100 and newer — __int128 single-instruction path.
  // Change to >= 800 if __int128 fails on SM 7.x (RTX 20xx).
  return (long long)((unsigned __int128)ua * ub % up);
#else
  // Fallback: __umul64hi + binary loop
  unsigned long long lo = ua * ub;
  unsigned long long hi = __umul64hi(ua, ub);

  // Fast path 1: no overflow
  ...
  unsigned long long hbase = c_pow2_64;
  while (hi) {
...
static vector<string> collect_files(const string &dir_path,
  ...
  while ((e = readdir(d)) != NULL) {
    ...
    // Extract kernel number from filename: kernel_38_0042.txt → 42
    string num_part = nm.substr(prefix.size());
    num_part = num_part.substr(0, num_part.size() - 4);  // remove .txt
    int knum = 0;
    try {
      knum = stoi(num_part);
    } catch (...) {
    ...
    // Filter by kernel range [kmin, kmax]
    if (knum < kmin || knum > kmax)
      continue;
  ...
  // Sort NUMERICALLY by kernel number, not lexicographically.
  // This handles both padded (0001) and unpadded (1) filenames correctly.
  sort(files.begin(), files.end(),
       [&prefix](const string &a, const string &b) {
    auto extract_num = [&prefix](const string &path) -> int {
      string fname = path.substr(path.find_last_of("/") + 1);
      string num = fname.substr(prefix.size());
      num = num.substr(0, num.size() - 4);  // remove .txt
...
static void write_group_result(int group, int best_dev, int best_hits,
                               bool all_hit, int kmin, int kmax,
                               int total_files, bool is_fixed_dev) {
  mkdir_safe(RESULT_BASE_DIR);
  string dir = string(RESULT_BASE_DIR) + "/" + to_string(group);
  mkdir_safe(dir);

  // Range suffix for multi-GPU: result_k1_k30.txt vs result.txt
  string range_suffix = "";
  if (kmax < INT_MAX) {
    range_suffix = "_k" + to_string(kmin) + "_k" + to_string(kmax);
  }
  string path = dir + "/result" + range_suffix + ".txt";
  ...
  fprintf(f, "============================================================\n");
  fprintf(f, "APM Result  (optimized_further)\n");
  fprintf(f, "============================================================\n");
  fprintf(f, "Prime group      : %d\n", group);
  if (kmax < INT_MAX)
    fprintf(f, "Kernel range     : %d to %d\n", kmin, kmax);
  fprintf(f, "Deviation        : %d\n", best_dev);
  fprintf(f, "Matrices in run  : %d\n", total_files);
  fprintf(f, "Matrices hit     : %d\n", best_hits);
  fprintf(f, "Hit ratio        : %.1f%%\n",
          total_files > 0 ? 100.0 * best_hits / total_files : 0.0);
  fprintf(f, "------------------------------------------------------------\n");

  if (is_fixed_dev) {
    // FIXED DEVIATION MODE (parallel chunk)
    fprintf(f, "Mode             : FIXED DEVIATION (parallel chunk)\n");
    if (all_hit)
      fprintf(f, "Status           : All %d matrices hit at deviation %d.\n",
              total_files, best_dev);
    else
      fprintf(f, "Status           : %d/%d matrices hit at deviation %d.\n",
              best_hits, total_files, best_dev);
    fprintf(f, "Note             : This is a partial result.\n");
    fprintf(f, "                   Run merge_summaries.py to combine all chunks.\n");
  } else {
    // AUTO-ITERATE MODE (sequential)
    fprintf(f, "Mode             : AUTO-ITERATE (sequential)\n");
    if (all_hit) {
      fprintf(f, "Status           : All matrices hit at deviation %d -- EARLY STOP.\n",
              best_dev);
...
static int run_group_deviation(int group, long long prime, int dev,
  ...
  // Build a suffix for summary files to avoid collisions in multi-GPU runs.
  // e.g., "_k1_k30" when running kernels 1-30, or "" when running all.
  string range_suffix = "";
  if (kmax < INT_MAX) {
    range_suffix = "_k" + to_string(kmin) + "_k" + to_string(kmax);
  }
  ...
  string det_path = out_dir + "/SUMMARY_detailed" + range_suffix + ".txt";
  ...
  string brief_path = out_dir + "/SUMMARY_brief" + range_suffix + ".txt";
...
int main(int argc, char **argv) {
  int gmin = 25, gmax = 60;
  int kmin = 1, kmax = INT_MAX;   // kernel file range (default: all)
  int fixed_dev = -1;             // -1 = auto-iterate all deviations
  ...
  if (argc == 3) {
    ...
    // ./apm_brahma 38 38 1 35
    gmin = atoi(argv[1]);
    gmax = atoi(argv[2]);
    kmin = atoi(argv[3]);
    kmax = atoi(argv[4]);
    if (gmin > gmax) { int tmp = gmin; gmin = gmax; gmax = tmp; }
    if (kmin > kmax) { int tmp = kmin; kmin = kmax; kmax = tmp; }
  } else if (argc == 6) {
    // ./apm_brahma 38 38 1 35 4   ← PARALLEL MODE: single deviation
    gmin = atoi(argv[1]);
    gmax = atoi(argv[2]);
    kmin = atoi(argv[3]);
    kmax = atoi(argv[4]);
    fixed_dev = atoi(argv[5]);
    if (gmin > gmax) { int tmp = gmin; gmin = gmax; gmax = tmp; }
    if (kmin > kmax) { int tmp = kmin; kmin = kmax; kmax = tmp; }
  } else if (argc != 1) {
    printf("Usage: %s [gmin gmax]                     (all kernels, all deviations)\n", argv[0]);
    printf("       %s [gmin gmax kmin kmax]           (kernel range, all deviations)\n", argv[0]);
    printf("       %s [gmin gmax kmin kmax dev]       (kernel range, SINGLE deviation)\n", argv[0]);
    printf("\n");
    printf("  Sequential (single GPU, auto-iterate deviations):\n");
    printf("    %s 38 38                   group 38, all kernels, all deviations\n", argv[0]);
    printf("    %s 25 50                   groups 25-50, all kernels\n", argv[0]);
    printf("\n");
    printf("  Parallel (multi-GPU, FIXED deviation):\n");
    printf("    %s 38 38  1 30 2           group 38, kernels 1-30, deviation 2\n", argv[0]);
    printf("    %s 38 38 31 55 2           group 38, kernels 31-55, deviation 2\n", argv[0]);
    printf("    %s 38 38 56 80 2           group 38, kernels 56-80, deviation 2\n", argv[0]);
    printf("    %s 38 38 81 100 2          group 38, kernels 81-100, deviation 2\n", argv[0]);
    return 1;
  }

  printf("=================================================================\n");
  printf(" APM Brahma — OPTIMIZED FURTHER\n");
  printf(" V100-OPTIMIZED, TEMPLATED, FLAT-ARRAY, KERNEL-RANGE VERSION\n");
  if (fixed_dev >= 0)
    printf(" MODE: FIXED DEVIATION (parallel-safe)\n");
  else
    printf(" MODE: AUTO-ITERATE DEVIATIONS (sequential)\n");
  ...
  printf("   - __int128 mod_mul on SM>=70, __umul64hi fallback otherwise\n");
  printf("   - __constant__ pow2_64 (L1-cached, zero param overhead)\n");
  printf("   - Flat int[] arrays (~5-8x VRAM savings)\n");
  printf("   - Numerical file sorting (1,2,...100 not 1,10,100,11...)\n");
  printf("   - Kernel range + deviation args for multi-GPU parallelism\n");
  printf("=================================================================\n\n");
  printf(" Group range     : %d to %d\n", gmin, gmax);
  if (kmax < INT_MAX)
    printf(" Kernel range    : %d to %d\n", kmin, kmax);
  else
    printf(" Kernel range    : ALL\n");
  if (fixed_dev >= 0)
    printf(" Deviation       : %d (FIXED)\n", fixed_dev);
  else
    printf(" Deviation       : AUTO (2 to max)\n");
  printf("\n");
  fflush(stdout);
  ...
  for (size_t gi = 0; gi < primes.size(); gi++) {
    ...
    string input_dir = "kernel_output/" + to_string(group);
    string prefix = "kernel_" + to_string(group) + "_";
    vector<string> files = collect_files(input_dir, prefix, kmin, kmax);

    if (files.empty()) {
      string alt_prefix = to_string(group) + "_";
      files = collect_files(input_dir, alt_prefix, kmin, kmax);
      if (!files.empty()) {
    ...
    // ================================================================
    // FIXED DEVIATION MODE (parallel-safe)
    // ================================================================
    if (fixed_dev >= 0) {
      int dev = fixed_dev;
      printf("  Deviation       = %d (FIXED)\n", dev);
      ...
      if (dev < MIN_DEV || dev > max_dev) {
        printf("  [SKIP] dev=%d out of range [%d, %d]\n", dev, MIN_DEV, max_dev);
        continue;
      }
      if (dev > n - PM_SIZE) {
      ...
      printf("\n  ===============================================================\n");
      printf("  [group=%d | dev=%d]  minor=%dx%d  |  %zu matrices\n",
             group, dev, PM_SIZE + dev, PM_SIZE + dev, files.size());
      printf("  ===============================================================\n");
      fflush(stdout);

      GPUBufs gpu;
      if (!gpu.alloc(n, dev)) {
        printf("  [SKIP] Insufficient VRAM for dev=%d\n", dev);
        continue;
      }

      int hits = run_group_deviation(group, prime, dev, files, n, gpu, kmin, kmax);
      printf("  [group=%d  dev=%d done]  hits=%d/%zu\n", group, dev, hits,
             files.size());
      fflush(stdout);

      write_group_result(group, dev, hits, (hits >= static_cast<int>(files.size())),
                         kmin, kmax, static_cast<int>(files.size()), true);

    // ================================================================
    // AUTO-ITERATE MODE (sequential single-GPU, original behavior)
    // ================================================================
    } else {
      ...
      for (int dev = MIN_DEV; dev <= max_dev; dev++) {
        ...
        // Dynamic early stop
        int stop_threshold = min(EARLY_STOP_HIT, static_cast<int>(files.size()));
        if (hits >= stop_threshold) {
          printf("\n  *** ALL %d/%zu MATRICES HIT at deviation %d! ***\n",
                 hits, files.size(), dev);
      ...
      if (best_dev >= 0)
        write_group_result(group, best_dev, best_hits, reached_target,
                           kmin, kmax, static_cast<int>(files.size()), false);
    }
